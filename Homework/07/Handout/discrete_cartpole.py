...
class DiscreteCartPoleEnv(gym.Env):
    def __init__(self, intervals=16):
        ...
        self._to_discrete = lambda x, a, b: int(
            min(max(0, (x - a) * self.intervals / (b - a)), self.intervals)
        )
...
class QLearner:
    ...
    def epsilon_decay(self, total_step):
        self.epsilon = self.epsilon_lower + (
            self.epsilon_upper - self.epsilon_lower
        ) * math.exp(-total_step / self.epsilon_decay_freq)

    def lr_decay(self, total_step):
        self.lr = self.lr_lower + (self.lr_upper - self.lr_lower) * math.exp(
            -total_step / self.lr_decay_freq
        )
    ...
    def update_q(self, total_step):
        ...
        for state, action, reward, new_state in batch:
            self.q[state][action] += self.lr * (
                self.gamma * self.q[new_state].max() + reward - self.q[state][action]
            )
...
if __name__ == "__main__":
    ...
    q_table = np.zeros(
        shape=(intervals + 1,) * env.observation_space.shape[0] + (env.action_space.n,)
    )
    ...
    if save_path not in os.listdir():
        ...
        latest_checkpoint = max(
            [int(file_name.split(".")[0]) for file_name in os.listdir(save_path)]
        )
