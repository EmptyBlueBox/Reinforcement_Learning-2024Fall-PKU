...
def reward_shaping(
    ...
    new_prev_treasure_dists = [dist for dist in prev_treasure_dists if dist != 1.0]
    new_treasure_dists = [dist for dist in treasure_dists if dist != 1.0]
    if len(new_prev_treasure_dists) > 0 and len(new_treasure_dists) > 0:    
        prev_nearest_dist = min(new_prev_treasure_dists)
        nearest_dist = min(new_treasure_dists) # 计算最近的宝箱, 删除所有1.0
        if prev_nearest_dist >= nearest_dist:
            reward_treasure_dist += 1

    # Reward 2.2 Reward for getting the treasure chest
    # 奖励2.2 获得宝箱的奖励
    reward_treasure = 0
    if prev_treasure_dists.count(1.0) < treasure_dists.count(1.0): # 收集的宝箱个数增加
        reward_treasure = 1
    ...
    # 奖励3.2 获得buff的奖励
    reward_buff = 0
    if prev_speed_up == 0 and speed_up == 1 and not buff_availability:
        reward_buff += 1
    ...
    if is_bump:
        reward_bump = 1 # 最后在 REWARD_CONFIG 再调成负的
