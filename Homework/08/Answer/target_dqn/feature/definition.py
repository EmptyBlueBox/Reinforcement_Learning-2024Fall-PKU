...
def reward_shaping(
    ...
    if prev_end_dist != 1:
        if num_collected_treasures >= max_treasures:
            reward_end_dist = 1 if end_dist < prev_end_dist else 0

    # Reward 1.2 Reward for winning
    # 奖励1.2 获胜的奖励
    reward_win = 0
    if terminated and (num_collected_treasures >= max_treasures):
        reward_win += 1
    ...
    # 奖励2.1 向宝箱靠近的奖励, 考虑最近的两个宝箱, 如果最近和第二近的宝箱距离都减小, reward 增加更多

    new_prev_treasure_dists = sorted([dist for dist in prev_treasure_dists if dist != 1.0])
    new_treasure_dists = sorted([dist for dist in treasure_dists if dist != 1.0])
    if len(new_treasure_dists) > 0 and len(new_prev_treasure_dists) > 0:
        min_treasure_dist = new_treasure_dists[0]
        min_prev_treasure_dist = new_prev_treasure_dists[0]
        if min_treasure_dist < min_prev_treasure_dist:
            reward_treasure_dist += 1
            if len(new_treasure_dists) > 1 and len(new_prev_treasure_dists) > 1:
                second_min_treasure_dist = new_treasure_dists[1]
                second_min_prev_treasure_dist = new_prev_treasure_dists[1]
                if second_min_treasure_dist < second_min_prev_treasure_dist:
                    reward_treasure_dist += 1
    ...
    # 奖励3.1 靠近buff的奖励
    reward_buff_dist = 0
    if prev_buff_dist != 1 and buff_availability:
        reward_buff_dist += 1 if buff_dist < prev_buff_dist else 0
    ...
    if num_collected_treasures >= max_treasures:
        ...
        reward_memory = np.mean(memory_map)
