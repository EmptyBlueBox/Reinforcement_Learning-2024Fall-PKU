...
public:
    ...
    MazeEnv(void *m,
            ...
            int s_x, int s_y,
            int g_x, int g_y,
            double step_r = 0.0, double goal_r = 1.0) : max_x(m_x), max_y(m_y),
    ...
    MazeEnv &operator=(const MazeEnv &m)
    {
        maze = new int[m.max_x * m.max_y];
        memcpy(maze, m.maze, sizeof(int) * m.max_x * m.max_y);
        return *this;
    }
    ...
    {
        return state.first >= 0 and state.first < max_x and state.second >= 0 and state.second < max_y and maze[locate(state.first, state.second)] == EMPTY;
    }
